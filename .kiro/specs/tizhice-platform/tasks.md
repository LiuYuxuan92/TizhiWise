...
- [ ] 9. 行为追踪、趋势分析与运营看板（R10）
  - [ ] 9.1 实现 BehaviorEvent 数据模型与采集接口
...
- [ ] 11. 内容管理、审批与发布导出（R8）
  - [ ] 11.1 实现 ContentDraft / ContentVersion 数据模型与迁移
...
- [ ] 13. H5 前端用户旅程（R1/R2/R3/R5/R6/R9/R10/R12）
  - [ ] 13.1 实现 H5 首页与双通道入口
    - 展示痛症测评、体质测评入口
  ...
  - [ ] 13.5 实现报告页、深度报告遮罩与付费解锁入口
    - 基础报告完整展示
    - 深度内容未付费时展示预览遮罩
    - 支付成功后刷新展示完整深度报告
    - _Requirements: 5.3, 5.4, 5.5_

  - [ ] 13.6 实现微信授权、我的中心与历史报告
    - 微信内授权登录
    - 匿名记录登录后归并
    - 我的中心展示测评、报告、订单
    - _Requirements: 9.1, 9.3, 9.4_

  - [ ] 13.7 实现微信分享、报告图片保存与埋点
    - 微信 JS-SDK 分享配置
    - 报告分享链接与导出图片调用
    - 关键用户行为事件埋点
