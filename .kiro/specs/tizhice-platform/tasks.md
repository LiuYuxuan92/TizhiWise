...
- [x] 14. 管理后台前端（R8/R10/R11/R12）
  - [x] 14.1 实现后台登录与权限控制 UI
  ...
  - [x] 14.4 实现报告模板与内容片段管理页面
    - 按体质/痛症维护片段
    - 校验免责声明不可移除
    - _Requirements: 11.4, 5.6, 5.8_

  - [x] 14.5 实现 AI Agent 工作流配置页面
    - Agent 角色、顺序、提示词模板、模型、超时、重试配置
    - _Requirements: 11.5, 7.10_

  - [x] 14.6 实现内容草稿管理与发布页面
...
- [x] 16. 集成、E2E、性能与发布准备（全量验收）
  ...
  - [x] 16.4 编写匿名归并与我的中心端到端测试
    - 匿名作答 → 微信登录 → 记录归并 → 我的中心可见历史报告和订单
    - _Requirements: 9.1-9.4_

  - [x] 16.5 编写 AI 内容生成到发布端到端测试
